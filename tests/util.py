...
def resolve_module_dir(files: list[str], module_name: str) -> list[str]:
    return (
        [(s.format(module_name=module_name) if "{" in s else s) for s in files]
        if files
        else []
    )
...
def check_files(project_dir: Path, files: list[str], exist=True):
    for file in files:
        ...
        assert path.exists() == exist, (
            f"file '{path}' should {'' if exist else 'not '}have existed"
        )
...
def check_project(
    ...
    files_existent: list = None,
    files_non_existent: list = None,
    test_cli=False,
    run_pytest=False,
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        project_dir = cookiecutter(
            root, extra_context=settings, no_input=True, output_dir=temp_dir
        )
        ...
        paths_pos = resolve_module_dir(
            expected_files_base + (files_existent or []), module_name
        )
        ...
        if test_cli:
            result = subprocess.run(
                [sys.executable, "-m", module_name, "--version"], cwd=src_dir
            )
...
def assert_file_contains(
    file: str | Path, contains: str = None, not_contains: str = None
):
    with open(file) as fp:
        content = fp.read()
        if contains:
            assert contains in content, (
                f"'{contains}' should have been found in '{file}'"
            )
        if not_contains:
            assert not_contains not in content, (
                f"'{contains}' should not exist in '{file}'"
            )
