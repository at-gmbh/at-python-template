...
def test_notebooks_yes():
    check_project(settings={"use_notebooks": "yes"}, files_existent=["notebooks"])


def test_notebooks_no():
    check_project(settings={"use_notebooks": "no"}, files_non_existent=["notebooks"])
...
def test_config_hocon():
    check_project(
        ...
        files_existent=[
            "src/{module_name}/util.py",
            "src/{module_name}/res/default.conf",
            "config/dev.conf",
            "config/prod.conf",
        ],
...
def test_formatter_black_pip():
    def check_black(project_dir: Path):
        assert_file_contains(
            project_dir / ".pre-commit-config.yaml", contains="psf/black"
        )
        assert_file_contains(project_dir / "requirements-dev.txt", contains="black")

    check_project(
        settings={"code_formatter": "black", "package_manager": "pip"}, fun=check_black
    )
...
def test_formatter_none():
    def check_white(project_dir: Path):
        assert_file_contains(
            project_dir / ".pre-commit-config.yaml", not_contains="psf/black"
        )
        assert_file_contains(project_dir / "environment-dev.yml", not_contains="black=")

    check_project(settings={"code_formatter": "none"}, fun=check_white)
...
def test_editor_none():
    check_project(
        settings={"editor_settings": "none"}, files_non_existent=[".idea", ".vscode"]
    )
...
def test_gitlab_pip():
    check_project(
        settings={"package_manager": "pip", "ci_pipeline": "gitlab"},
...
def test_gitlab_conda():
    check_project(
        settings={"package_manager": "conda", "ci_pipeline": "gitlab"},
        files_existent=[".gitlab-ci.yml"],
    )
...
def test_gitlab_poetry():
    check_project(
        settings={"package_manager": "poetry", "ci_pipeline": "gitlab"},
        files_existent=[".gitlab-ci.yml"],
    )
...
def test_no_ci_pipeline():
    check_project(
        settings={"ci_pipeline": "none"}, files_non_existent=[".gitlab-ci.yml"]
    )
