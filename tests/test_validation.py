...
def test_module_name():
    with pytest.raises(CookiecutterException):
        check_project(
            settings={"module_name": "don't you tell *me* how to name *my* modules!"}
        )
...
def fail_python_version(mocker: MockerFixture, version_info: str):
    patch_cookiecutter_run_script(
        mocker,
        lambda mocker: mocker.patch("platform.python_version", lambda: version_info),
    )
...
def patch_cookiecutter_run_script(
    mocker: MockerFixture, patch_fun: Callable[[MockerFixture], Any]
):
